...
class SaferEval {
    ...
    var $source, $allowedCalls, $allowedTokens, $allowedVariables, $disallowedExpressions, $errors, $tokens, $braces, $code;
    ...
    function checkScript($code, $execute, $strict = false) {
        ...
        if (empty($this->errors)) {
            ...
            foreach ($this->tokens as $key => $token) {
                ...
                if (is_array($token)) {
                    ...
                    switch ($id) {
                        case('T_VARIABLE'):
                            $this->allowedVariables($this->tokens, $i, $strict);
                            break;
                        case('T_STRING'):
                            $this->allowedCalls($this->tokens, $i, $strict);
                            break;
                        default:
                            $this->allowedTokens($this->tokens, $i, $id, $strict);
                            break;
                    }
                }
            }
    ...
    function allowedVariables($token, $i, $strict = false)
    {
        ...
        if (in_array($token[1], $allowedVariables) === false) {
            $this->errors[$i]['name'] = 'Illegal variable: ' . $token[1];
            $this->errors[$i]['line'] = $token[2];
        }
    }

    function allowedCalls($token, $i, $strict = false)
    {
    ...
    function allowedTokens($token, $i, $id, $strict = false)
    {
